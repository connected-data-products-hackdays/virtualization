...
def _construct_manifest_array(
    ...
    """
    shape = dataset.shape
    axes = tuple(range(len(dataset.shape)))
    chunks = dataset.chunks or dataset.shape
    codecs = codecs_from_dataset(dataset)
    attrs = _extract_attrs(dataset)
    dtype = dataset.dtype

    if any(d == 1 for d in dataset.shape):
        axes = tuple(i for i, a in enumerate(dataset.shape) if a != 1)
        shape = tuple(a for i, a in enumerate(dataset.shape) if i in axes)
        chunks = shape  # TODO[reece]: probably check chunks separately
    ...
    try:
        ...
        if isinstance(dataset.fillvalue, h5py.h5r.Reference):
            ref_name = h5py.h5r.get_name(dataset.fillvalue, dataset.id).decode()
            print(f"error on ref {ref_name}")
            fill_value = None
            # raise ValueError(f"Error on ref {ref_name}")
        raise ValueError(f"fillvalue error on variable {dataset.name}")
    dims = tuple(_dataset_dims(dataset, group=group))
    dims = tuple(dim for i, dim in enumerate(dims) if i in axes)  # filter down to remaining dimensions after squeeze
